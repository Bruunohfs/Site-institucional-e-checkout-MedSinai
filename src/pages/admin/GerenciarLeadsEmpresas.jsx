...
// --- COMPONENTE DE LINHA/CARD (COM A NOVA REGRA) ---
function LeadRow({ lead, onUpdate, userEmail }) {
  const [editableLead, setEditableLead] = useState({ ...lead });
  const [isSaving, setIsSaving] = useState(false);

  // =====> MUDANÇA: LÓGICA DO DROPDOWN DE STATUS <=====
  // A lista base de status
  const baseStatusOptions = ['novo', 'em negociação', 'fechado', 'perdido'];
  
  // Determina quais opções mostrar no dropdown
  // Se o status original do lead (lead.status) NÃO for 'novo', remove a opção 'novo' da lista.
    ...
    <>
      ...
      <tr className="hidden md:table-row bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 align-top">
        ...
        <td className="px-4 py-4 space-y-2">
          ...
          <textarea name="anotacoes" value={editableLead.anotacoes || ''} onChange={handleChange} rows="2" className="w-full p-2 border rounded-md bg-gray-50 dark:bg-gray-700 dark:border-gray-600 text-sm" placeholder="Adicionar anotação..."/>
        </td>
      ...
      <div className="md:hidden p-4 border-b dark:border-gray-700">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <textarea name="anotacoes" value={editableLead.anotacoes || ''} onChange={handleChange} rows="3" className="w-full p-2 border rounded-md bg-gray-50 dark:bg-gray-700 dark:border-gray-600 text-sm mt-1" placeholder="Adicionar anotação..."/>
          </div>
...
function GerenciarLeadsEmpresas() {
  ...
  }, [filter, user]); // Adicionado 'user' como dependência
    ...
    <div className="p-4 sm:p-6 lg:p-8 bg-gray-100 dark:bg-gray-900 min-h-screen">
      <div className="max-w-full mx-auto">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <button onClick={() => setFilter('em negociação')} className={`px-4 py-2 text-sm font-medium rounded-md ${filter === 'em negociação' ? 'bg-purple-600 text-white' : 'bg-white dark:bg-gray-700'}`}>Em Negociação</button>
          <button onClick={() => setFilter('fechado')} className={`px-4 py-2 text-sm font-medium rounded-md ${filter === 'fechado' ? 'bg-green-600 text-white' : 'bg-white dark:bg-gray-700'}`}>Fechados</button>
          <button onClick={() => setFilter('perdido')} className={`px-4 py-2 text-sm font-medium rounded-md ${filter === 'perdido' ? 'bg-red-600 text-white' : 'bg-white dark:bg-gray-700'}`}>Perdidos</button>
        </div>
              ...
              <div className="hidden md:block">
                <table className="min-w-full divide-y divide-gray-300 dark:divide-gray-700">
                  <thead className="bg-gray-300 dark:bg-gray-700/50">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-1/4">Lead</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-1/4">Contato</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-2/4">Gerenciamento</th>
                  ...
                  <tbody className="divide-y divide-gray-300 dark:divide-gray-700">
                    {leads.map((lead) => (
                      <LeadRow key={lead.id} lead={lead} onUpdate={handleLeadUpdate} userEmail={user?.email} />
                    ))}
                  </tbody>
                </table>
              </div>
