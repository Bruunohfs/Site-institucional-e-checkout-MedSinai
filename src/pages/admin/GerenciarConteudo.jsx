...
import EditMaterialModal from './EditMaterialModal'; // Importa o modal de edição
...
export default function GerenciarConteudo() {
  ...
  const renderContent = () => {
    ...
    if (materiais.length === 0) return <div className="text-center py-10 text-gray-500">Nenhum material cadastrado.</div>;
      ...
      <table className="w-full text-left">
        <thead className="bg-gray-700/50">
          <tr>
            ...
            <th className="p-4 font-semibold text-white">Data de Criação</th>
            <th className="p-4 font-semibold text-white">Ações</th>
          </tr>
        </thead>
        <tbody>
          {materiais.map(material => (
            <tr key={material.id} className="border-b border-gray-700 last:border-b-0 hover:bg-gray-700/50 transition-colors">
              <td className="p-4 text-white font-medium">{material.titulo}</td>
              <td className="p-4 text-gray-400 capitalize">{material.tipo}</td>
              <td className="p-4 text-gray-400">{new Date(material.created_at).toLocaleDateString('pt-BR')}</td>
              <td className="p-4 flex gap-2">
                <button onClick={() => handleOpenEditModal(material)} className="p-2 rounded-md hover:bg-gray-600 text-gray-300 transition-colors">
                  <EditIcon />
                </button>
                <button onClick={() => handleDeleteMaterial(material)} className="p-2 rounded-md hover:bg-red-900/50 text-red-400 transition-colors">
                  <DeleteIcon />
                </button>
              </td>
            </tr>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setIsAddModalOpen(true)} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg">
          <AddIcon />
          Novo Material
        </button>
      </div>
