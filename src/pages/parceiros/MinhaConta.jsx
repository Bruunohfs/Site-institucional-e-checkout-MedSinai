// src/pages/parceiros/MinhaConta.jsx - VERSÃO FINAL COM VALIDAÇÃO DE SENHA FORTE
...
import { User, Landmark, KeyRound, Save, XCircle, CheckCircle2 } from 'lucide-react'; // <-- Ícones para o validador
...
// ===================================================================
// ==> PASSO 1: NOVO COMPONENTE DE FEEDBACK DE SENHA <==
// ===================================================================
...
export default function MinhaContaPage() {
  ...
  const { register: registerPassword, handleSubmit: handleSubmitPassword, formState: { errors: errorsPassword }, watch: watchPassword, reset: resetPassword } = useForm({ mode: 'onChange' }); // <-- mode: 'onChange' para feedback em tempo real
  ...
  const newPasswordValue = watchPassword('newPassword'); // Observa o valor da nova senha
  ...
  useEffect(() => {
    ...
    fetchProfile();
  }, [user, setValueInfo, setValueBanco, addNotification]);

  const onUpdateInfo = async (data) => { const { error } = await supabase.from('profiles').update({ nome_completo: data.nome_completo, telefone: data.telefone }).eq('id', user.id); if (error) { addNotification(`Erro ao atualizar dados: ${error.message}`, 'error'); } else { addNotification('Dados pessoais atualizados com sucesso!', 'success'); window.dispatchEvent(new CustomEvent('profileUpdated')); } };
  const onUpdateBanco = async (data) => { const { error } = await supabase.from('profiles').update({ tipo_chave_pix: data.tipo_chave_pix, chave_pix: data.chave_pix, nome_banco: data.nome_banco, agencia: data.agencia, conta_corrente: data.conta_corrente }).eq('id', user.id); if (error) { addNotification(`Erro ao atualizar dados bancários: ${error.message}`, 'error'); } else { addNotification('Dados bancários atualizados com sucesso!', 'success'); } };
  const onUpdatePassword = async (data) => { const { error: signInError } = await supabase.auth.signInWithPassword({ email: user.email, password: data.currentPassword }); if (signInError) { addNotification('A sua senha atual está incorreta.', 'error'); return; } const { error: updateError } = await supabase.auth.updateUser({ password: data.newPassword }); if (updateError) { let friendlyMessage = updateError.message.includes('New password should be different') ? 'A nova senha não pode ser igual à senha antiga.' : `Erro: ${updateError.message}`; addNotification(friendlyMessage, 'error'); } else { addNotification('Senha alterada com sucesso!', 'success'); resetPassword(); } };
      ...
      <div className="space-y-8">
        ...
        <SectionCard title="Dados de Pagamento" icon={<Landmark className="w-6 h-6 text-green-500" />}>
          ...
          <form onSubmit={handleSubmitBanco(onUpdateBanco)} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label htmlFor="tipo_chave_pix" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tipo de Chave PIX</label><select id="tipo_chave_pix" {...registerBanco('tipo_chave_pix')} className="w-full p-2.5 rounded-md border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 focus:outline-none transition"><option value="">Selecione um tipo</option><option value="CPF/CNPJ">CPF/CNPJ</option><option value="Email">E-mail</option><option value="Telefone">Telefone</option><option value="Aleatória">Chave Aleatória</option></select></div><FormField label="Chave PIX" id="chave_pix" register={registerBanco('chave_pix')} error={errorsBanco.chave_pix} /></div>
            <p className="text-center text-xs text-gray-400 my-4">Ou, se preferir, preencha os dados para TED/DOC:</p>
            <div><label htmlFor="nome_banco" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nome do Banco</label><select id="nome_banco" {...registerBanco('nome_banco')} className="w-full p-2.5 rounded-md border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 focus:outline-none transition"><option value="">Selecione seu banco</option>{LISTA_BANCOS.map(banco => ( <option key={banco.codigo} value={banco.nome}>{banco.codigo} - {banco.nome}</option>))}</select></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label htmlFor="agencia" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Agência (sem dígito)</label><Controller name="agencia" control={controlBanco} render={({ field: { onChange, name, value } }) => ( <IMaskInput mask={'0000'} id={name} name={name} value={value || ''} placeholder="Ex: 1234" className="w-full p-2.5 rounded-md border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 focus:outline-none transition" onAccept={(val) => onChange(val)} /> )} /></div><div><label htmlFor="conta_corrente" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Conta Corrente (com dígito)</label><Controller name="conta_corrente" control={controlBanco} render={({ field: { onChange, name, value } }) => ( <IMaskInput mask={'00000000-0'} id={name} name={name} value={value || ''} placeholder="Ex: 12345-6" className="w-full p-2.5 rounded-md border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 focus:outline-none transition" onAccept={(val) => onChange(val)} /> )} /></div></div>
        ...
        <SectionCard title="Alterar Senha" icon={<KeyRound className="w-6 h-6 text-orange-500" />}>
          <form onSubmit={handleSubmitPassword(onUpdatePassword)} className="space-y-4">
            ...
            {/* =================================================================== */}
            {/* ==> PASSO 2: APLICAR AS NOVAS REGRAS DE VALIDAÇÃO <== */}
            {/* =================================================================== */}
            <FormField 
              label="Nova Senha" 
              id="newPassword" 
              ...
              register={registerPassword('newPassword', { 
                ...
                validate: {
                  hasUpper: value => /[A-Z]/.test(value) || 'A senha deve conter uma letra maiúscula.',
                  hasLower: value => /[a-z]/.test(value) || 'A senha deve conter uma letra minúscula.',
                  hasNumber: value => /[0-9]/.test(value) || 'A senha deve conter um número.',
                  hasSpecialChar: value => /[^A-Za-z0-9]/.test(value) || 'A senha deve conter um caractere especial.',
                }
              })} 
            ...
            {/* =================================================================== */}
            {/* ==> PASSO 3: RENDERIZAR O COMPONENTE DE FEEDBACK <== */}
            {/* =================================================================== */}
