...
const FinanceIcon = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 17h8m0 0V9m0 8l-8-8-4 4-6-6"></path></svg>;


export default function Sidebar({ user, onLogout, theme, onThemeSwitch, isOpen, setIsOpen }) {
  // 3. CRIAR UM ESTADO LOCAL PARA O NOME
  const [displayName, setDisplayName] = useState('Parceiro');

  // 4. EFEITO PARA BUSCAR O NOME DO PERFIL E OUVIR O EVENTO
  useEffect(() => {
    ...
    const fetchProfileName = async () => {
      if (user?.id) {
        ...
        // Se encontrar um nome no perfil, usa ele. Senão, usa o nome do metadata antigo.
        const finalName = profile?.nome_completo || user.user_metadata?.nome || 'Parceiro';
        setDisplayName(finalName);
      }
    };

    fetchProfileName(); // Busca o nome quando o componente carrega

    // Função que será chamada quando o evento 'profileUpdated' for disparado
    const handleProfileUpdate = () => {
      fetchProfileName(); // Busca o nome novamente
    };

    // Adiciona o "ouvinte" de eventos
    window.addEventListener('profileUpdated', handleProfileUpdate);

    // Limpa o "ouvinte" quando o componente for desmontado (muito importante!)
    return () => {
      window.removeEventListener('profileUpdated', handleProfileUpdate);
    };
  }, [user]); // Roda este efeito sempre que o objeto 'user' mudar
      ...
      >
        <div className="flex flex-col h-full bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700">
          ...
          <nav className="flex-grow p-4 space-y-2">
            <NavLink to="/parceiros/dashboard" end onClick={handleLinkClick} className={({ isActive }) => `${linkStyle} ${isActive ? activeLinkStyle : inactiveLinkStyle}`}><HomeIcon /> Visão Geral</NavLink>
            ...
            <NavLink to="/parceiros/financeiro" onClick={handleLinkClick} className={({ isActive }) => `${linkStyle} ${isActive ? activeLinkStyle : inactiveLinkStyle}`}><FinanceIcon /> Meu Financeiro</NavLink>
