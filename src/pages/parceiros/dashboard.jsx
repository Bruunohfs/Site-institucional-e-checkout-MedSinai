...
export default function DashboardPage() {
  ...
  const mesesDisponiveis = useMemo(() => {
    ...
    vendas.forEach(venda => {
      if (venda.status_pagamento !== 'CONFIRMED' && venda.status_pagamento !== 'RECEIVED') return;
      const isParcelada12x = venda.nome_plano && venda.nome_plano.includes('12x');
      const dataCompetencia = isParcelada12x ? venda.data_vencimento : venda.data_pagamento;
      if (dataCompetencia) {
  ...
  const { dataInicio, dataFim } = useMemo(() => {
    ...
    } else if (filtroPeriodo.match(/^\d{4}-\d{2}$/)) {
  ...
  const vendasProcessadas = useMemo(() => {
    const vendasComCompetencia = vendas.map(v => {
      const isParcelada12x = v.nome_plano && v.nome_plano.includes('12x');
      return { ...v, data_competencia: isParcelada12x ? v.data_vencimento : v.data_pagamento };
    });

    return vendasComCompetencia
      .filter(venda => {
        if (venda.status_pagamento !== 'CONFIRMED' && venda.status_pagamento !== 'RECEIVED') {
          return false;
        }
        if (!venda.data_competencia) return filtroPeriodo === 'all';
        
        const dataComp = new Date(venda.data_competencia);
        const dataCompSemHora = new Date(dataComp.getUTCFullYear(), dataComp.getUTCMonth(), dataComp.getUTCDate());

        return dataCompSemHora >= dataInicio && dataCompSemHora <= dataFim;
      })
      .sort((a, b) => { 
  ...
  }, [vendas, sortConfig, dataInicio, dataFim]);

  // ===================================================================
  // ==> CORREÇÃO FINAL DOS KPIs <==
  // ===================================================================
  const kpis = useMemo(() => {
    ...
    vendasProcessadas.forEach(venda => {
      comissaoTotal += venda.valor * TAXA_COMISSAO;
      const vendaId = venda.id_cobranca_principal || venda.id_pagamento || venda.id;
      if (vendaId) {
        idsVendasUnicas.add(vendaId);
      }
    });

    return {
      comissaoAprovadaPeriodo: comissaoTotal,
      vendasPagasPeriodo: idsVendasUnicas.size,
    };
  }, [vendasProcessadas]); // Agora depende de vendasProcessadas!
  ...
  const vendasPaginadas = useMemo(() => {
    ...
    return vendasProcessadas.slice(firstPageIndex, firstPageIndex + ITENS_POR_PAGINA);
      ...
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="flex flex-wrap justify-between items-center gap-4 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select 
              id="month-selector"
              value={filtroPeriodo.match(/^\d{4}-\d{2}$/) ? filtroPeriodo : 'default'}
              onChange={(e) => setFiltroPeriodo(e.target.value)} 
              className="px-3 py-1.5 text-sm rounded-md bg-gray-300 dark:bg-gray-700 border-gray-300 dark:border-gray-600"
            >
          ...
          {totalPages > 1 && (
            <div className="flex justify-between items-center p-4 border-t border-gray-200 dark:border-gray-700">
              ...
              <div className="flex gap-2">
                <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3 py-1 bg-gray-200 dark:bg-gray-700 rounded-md text-sm disabled:opacity-50 hover:bg-gray-300 dark:hover:bg-gray-600">Anterior</button>
                <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-3 py-1 bg-gray-200 dark:bg-gray-700 rounded-md text-sm disabled:opacity-50 hover:bg-gray-300 dark:hover:bg-gray-600">Próxima</button>
              </div>
            </div>
          )}
        </div>
