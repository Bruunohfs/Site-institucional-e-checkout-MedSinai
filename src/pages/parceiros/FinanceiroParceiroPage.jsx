...
// --- ESTILOS DE STATUS ---
const STATUS_STYLES = {
  PAGO_TOTAL: 'bg-green-400 text-green-800 dark:bg-green-500/20 dark:text-green-400',
  PAGO_PARCIAL: 'bg-blue-400 text-blue-800 dark:bg-blue-500/20 dark:text-blue-400',
  PENDENTE: 'bg-yellow-400 text-yellow-800 dark:bg-yellow-500/20 dark:text-yellow-400',
  DEFAULT: 'bg-gray-400 text-gray-800 dark:bg-gray-600 dark:text-gray-300'
};
const getStatusClasses = (status) => STATUS_STYLES[status] || STATUS_STYLES.DEFAULT;

// --- COMPONENTE DetalhesVendasRow (LÓGICA JÁ CORRIGIDA) ---
function DetalhesVendasRow({ fechamento, user }) {
  ...
  useEffect(() => {
    const fetchVendasDoMesCorretamente = async () => {
      ...
      try {
        ...
        console.error("Erro ao buscar vendas do extrato do parceiro:", error);
        setVendas([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <tr className="bg-gray-300 dark:bg-gray-900/50">
      <td colSpan="5" className="p-4 space-y-4">
        {fechamento.observacoes && (
          <div className="p-4 bg-gray-200 dark:bg-gray-800 rounded-lg mb-4">
            <h4 className="font-bold text-md mb-2 text-gray-800 dark:text-white">Observações:</h4>
            <p className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{fechamento.observacoes}</p>
          </div>
        )}
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="text-left text-gray-900 dark:text-gray-400">
                  <tr>
                    <th className="py-2 px-3">Cliente</th>
                    <th className="py-2 px-3">Plano</th>
                    <th className="py-2 px-3 text-right">Valor da Venda</th>
                    <th className="py-2 px-3 text-right">Sua Comissão (40%)</th>
                    <th className="py-2 px-3 text-right">Data de Competência</th>
                  </tr>
                </thead>
                <tbody>
                  {vendas.map(venda => (
                    <tr key={venda.id} className="border-t border-gray-300 dark:border-gray-700">
...
// --- COMPONENTE PRINCIPAL (COM A COLUNA "DETALHES" CORRIGIDA) ---
              ...
              {fechamentos.length > 0 ? fechamentos.map(f => (
                <React.Fragment key={f.id}>
                  <tr className="border-t border-gray-400 dark:border-gray-700">
                    ...
                    {/* =================================================================== */}
                    {/* ==> CORREÇÃO FINAL NA COLUNA DETALHES <== */}
                    {/* =================================================================== */}
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        {/* O botão de expandir agora aparece sempre */}
                        <button onClick={() => toggleRowExpansion(f.id)} title="Ver Extrato Detalhado" className={`p-1 rounded-full text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700 transition-transform transform ${expandedRowId === f.id ? 'rotate-180' : ''}`}>
                          <ChevronDownIcon />
                        </button>
                        {/* O ícone do comprovante só aparece se a URL existir */}
                        {f.url_comprovante && (
                        ...
                        {/* O ícone de chat (observações) só aparece se houver texto */}
                        {f.observacoes && (
                           <span title={f.observacoes} className="cursor-help text-gray-500 hover:text-gray-400"><ChatIcon /></span>
                        )}
                      </div>
                    </td>
                  </tr>
                  {/* A linha de detalhes é renderizada condicionalmente aqui */}
