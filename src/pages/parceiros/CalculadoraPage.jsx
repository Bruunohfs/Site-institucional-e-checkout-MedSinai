...
const ResultCard = ({ title, value, isAnnual = false }) => ( <div className="bg-green-100 dark:bg-green-900/50 p-6 rounded-2xl text-center shadow-lg border border-green-200 dark:border-green-700"><h3 className="text-lg font-semibold text-green-800 dark:text-green-200">{title}</h3><p className="text-4xl font-bold text-green-600 dark:text-green-400 mt-2">{formatCurrency(value)}</p>{isAnnual && <p className="text-sm text-green-700 dark:text-green-300">({formatCurrency(value / 12)} / mês)</p>}</div> );
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-12">
      ...
      <section>
        ...
        <ReverseCalculator />
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8"><ResultCard title="Potencial de Comissão" value={totalComissaoCpf} /><ResultCard title="Potencial de Ganho Anual" value={totalComissaoCpf * 12} isAnnual={true} /></div>
      ...
      <section>
        <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-semibold text-gray-800 dark:text-white border-l-4 border-purple-500 pl-4">Comparador de Cenários</h2><button onClick={handleSaveCenario} className="px-4 py-2 bg-purple-600 text-white font-semibold rounded-lg shadow-md hover:bg-purple-700 transition-colors">Salvar Simulação Atual</button></div>
        {cenarios.length > 0 ? (<div className="flex gap-6 overflow-x-auto pb-4">{cenarios.map(cenario => (<CenarioCard key={cenario.id} cenario={cenario} onRemove={handleRemoveCenario} />))}</div>) : (<div className="text-center py-10 px-6 bg-white dark:bg-gray-800 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700"><p className="text-gray-500 dark:text-gray-400">Faça uma simulação nas calculadoras acima e clique em "Salvar Simulação Atual" para começar a comparar suas estratégias de venda.</p></div>)}
      </section>
